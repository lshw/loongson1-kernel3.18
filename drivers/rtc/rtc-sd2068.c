		...
		const struct i2c_device_id *id)
{
	...
	sd2068 = devm_kzalloc(&client->dev, sizeof(struct sd2068), GFP_KERNEL);
	if (!sd2068)
		return -ENOMEM;
	...
	if (client->irq > 0) {
		ret = devm_request_irq(&client->dev, client->irq, sd2068_irq,
						IRQF_SHARED, "sd2068", client);
		if (ret) {
	...
	sd2068->rtc = devm_rtc_device_register(&client->dev, client->name,
					  &sd2068_rtc_ops, THIS_MODULE);
	return PTR_ERR_OR_ZERO(sd2068->rtc);
}
...
module_i2c_driver(sd2068_driver);
